  ...
  */
  ...
  const showVisualization = useCallback((el) => {
    ...
    /*
       ...
       - Create the embedded query by navigating to an Explore in your Looker interface (select an explore from the Explore tab in the left navigation menu).
       - Open the visualization dropdown in the Explore. Select the fields, filters, visualization type, and visualization preferences and hit run to see the visual.
       - When the visual looks as desired, get the expanded query url:
          ...
          - Copy the 'Expanded URL' for the explore: [instance].looker.com/explore/[copy the expanded URL here]
          - Update the URL passed into LookerEmbedSDK.createExploreWithUrl() below with the new expanded URL:  ${hostUrl}/embed/query/[paste the new expanded URL here]
