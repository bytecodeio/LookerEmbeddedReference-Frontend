  ...
  const showVisualization = useCallback((el) => {
    if (el) {
      ...
      /*
        ...
        The long url here includes all the options for the explore and visualization. 
        You can fetch the query fields by configuring a chart in an explore, then using the explore menu to 'share'.
        The 'expanded url there contains the query fields you need.
        
        Alternatively, you can simply use the syntax with a query id (qid) from an explore:
           /embed/query/data_block_acs_bigquery/acs_census_data?qid=ZmNZZBUZUOjTI8PZudiXCT&sdk=2&embed_domain=${hostUrl}&sandboxed_host=true
        */     
